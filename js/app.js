...
function initDealCarrousel(dealCarrouselID) {
    ...
    function updateUpaCarrouselInfo() {
        cardOutterWidth = document.querySelector("#" + dealCarrouselID + " .va-card").offsetWidth + 30; //you can define how far the scroll
...
const btnEle = document.querySelector('#storeEmail');
const inptEle = document.querySelector('.entered__email');

let count = 1;

btnEle.addEventListener('click', () => {
    if (inptEle.value !== '') {
        window.localStorage.setItem('email' + count, inptEle.value);
        inptEle.value = '';
        count++;
    }
});